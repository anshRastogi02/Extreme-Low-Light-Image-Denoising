...
low_image_files = sorted(glob(os.path.join(r"C:\Denoising\Extreme-Low-Light-Image-Denoising\dataset\test\low", "*.png")))
...
for original_image in (low_image_files):
    ...
    original_image = np.array(original_image).astype(np.float32)/float(pixel)
    output_image = np.array(output_image).astype(np.float32)/float(pixel)
